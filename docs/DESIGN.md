...
| **Permission Dialog** | 🔴 **三灯闪烁** | Auto hook `PermissionRequest[Bash]` | 权限弹窗等待用户决策时 |
| **Permission Denied** | 🔴 **红灯** | Auto hook `PermissionDenied[Bash]` | 权限请求被拒绝时 |
| **Need Input** | 🔴 **红灯** | 手动 `traflight red` | Agent 需要用户决策时 |
| **Error** | 🔴 **闪烁** | Auto hook `PostToolUseFailure` | 命令执行失败时 |
| **Notification** | 🔴 **三灯闪烁** | Auto hook `Notification` | 桌面通知时 |
...
```
                  ...
                  │  PermissionRequest → 🔴 blink_all     │
                  │  PermissionDenied  → 🔴 red           │
                  │  Notification      → 🔴 blink_all     │
                  └────────────────────────────────────┘
...
```json
{"cmd":"blink_all","times":3,"interval":500}
```

同时闪烁红、黄、绿三灯，持续闪烁直到下一个 `light` / `blink` / `pattern` 指令停止。
`times` 参数当前被忽略（无限闪烁），保留用于未来版本。
...
```json
{
  "hooks": {
    ...
    "PermissionRequest": [
      { "matcher": "Bash(*)", "hooks": [
          {"type":"command", "command":"bash traflight-hook.sh permission",
           "async": true, "timeout": 5}
      ]}
    ],
    "PermissionDenied": [
      { "matcher": "Bash(*)", "hooks": [
          {"type":"command", "command":"bash traflight-hook.sh denied",
           "async": true, "timeout": 5}
      ]}
    ]
  }
...
├── setup()
│   ├── Serial.begin(115200)
│   ├── tft.init() + setRotation(2)  // 旋转 180° 适配机箱方向
│   ├── self-test (红→黄→绿自检动画)
│   └── 打印启动信息
│
└── loop()
