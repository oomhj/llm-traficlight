...
- 红绿灯仅用于 Agent 状态指示（auto hooks 控制）
- CPU/MEM 显示条通过 `bash traflight-daemon.sh health` 更新
- 红绿灯和监控面板功能完全解耦
...
```bash
# 一次性采集并更新显示
bash traflight-daemon.sh health

# 后台持续监控 (每 5 秒更新)
bash traflight-daemon.sh healthd
bash traflight-daemon.sh health-stop
```

监控数据通过 daemon FIFO 队列发送，不直接操作串口，避免与 hooks 竞争。
...
- **红绿灯**: 横向排列，长边撑满 128px。点亮有辉光+高光
- **监控条**: 标题 | 20 格条形图 | 百分比，一行一条
- 红绿灯和监控面板功能完全解耦，互不影响
